...
Full script with independent colormaps for rho(+P) and rho(-P).

Usage example:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
python3 2Heat_3.125Zr.py \
  --plus-den  "/home/yanan/Material/Y_dope_project/3.125Per-Zr-HfO2-2x2x2/berry-scf-Results/berry_PlusP_3.125Zr_HfO2_12247641/berry_PlusP_3o_DEN.nc" \
  --minus-den "/home/yanan/Material/Y_dope_project/3.125Per-Zr-HfO2-2x2x2/berry-scf-Results/berry_MinusP_3.125Zr_HfO2_12247642/berry_MinusP_3o_DEN.nc" \
  --plus-in   "/home/yanan/Material/Y_dope_project/3.125Per-Zr-HfO2-2x2x2/berry-scf-Results/berry_PlusP_3.125Zr_HfO2_12247641/berry_PlusP_3.125Per_Zr_HfO2_2x2x2.in" \
  --minus-in  "/home/yanan/Material/Y_dope_project/3.125Per-Zr-HfO2-2x2x2/berry-scf-Results/berry_MinusP_3.125Zr_HfO2_12247642/berry_MinusP_3.125Per_Zr_HfO2_2x2x2.in" \
  --units ang \
  --outdir 2_DEN_Figures_SHARP_3.125Zr \
  --write-cube \
  --cmap-plus viridis \
  --cmap-minus plasma \
  --cmap-drho seismic
...
Notes:
- rho(+P) uses --cmap-plus
- rho(-P) uses --cmap-minus
- Δrho uses --cmap-drho and ALWAYS has symmetric vmin/vmax about 0
- You can reverse any colormap with --reverse-plus / --reverse-minus / --reverse-drho
"""
...
def _maybe_reverse_cmap(cmap: Optional[str], reverse: bool) -> Optional[str]:
    if cmap is None:
        return None
    if reverse:
        return cmap if cmap.endswith("_r") else f"{cmap}_r"
    return cmap
...
def main():
    ...
    ap.add_argument("--outdir", default="3_DEN_Figures_SHARP_3.125Zr", help="Base output directory name")
    ...
    # NEW: independent colormaps for rho(+P) and rho(-P)
    ap.add_argument("--cmap-plus", default="viridis",
                    help="Colormap for rho(+P). Examples: viridis, plasma, magma, cividis, Greys")
    ap.add_argument("--cmap-minus", default="plasma",
                    help="Colormap for rho(-P). Examples: viridis, plasma, magma, cividis, Greys")
    ap.add_argument("--reverse-plus", action="store_true", help="Reverse rho(+P) colormap (adds _r).")
    ap.add_argument("--reverse-minus", action="store_true", help="Reverse rho(-P) colormap (adds _r).")

    # (Optional but useful) Δrho colormap control
    ap.add_argument("--cmap-drho", default="RdBu_r",
                    help="Colormap for delta rho. Examples: RdBu_r, seismic, coolwarm, PuOr")
    ap.add_argument("--reverse-drho", action="store_true", help="Reverse delta-rho colormap (adds _r).")

    args = ap.parse_args()

    # Ensure output folder includes 3.125Zr
    ...
    # Δrho symmetric color scale ALWAYS
    dv = float(np.max(np.abs(delta2)))
    if not np.isfinite(dv) or dv == 0.0:
        dv = 1.0

    cmap_plus = _maybe_reverse_cmap(args.cmap_plus, args.reverse_plus)
    cmap_minus = _maybe_reverse_cmap(args.cmap_minus, args.reverse_minus)
    cmap_drho = _maybe_reverse_cmap(args.cmap_drho, args.reverse_drho)
