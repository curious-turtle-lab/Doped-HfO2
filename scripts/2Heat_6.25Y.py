...
Like 2Heat_6.25Y.py, but ALSO supports ABINIT binary DEN files without ".nc".
If input is *_DEN (no .nc), we auto-convert with cut3d -> NetCDF, then proceed.

Example (your 6.25% Y case with DEN *without* .nc):
#############################################################################
python3 2Heat_6.25Y.py \
  --plus-den  "/home/yanan/Material/Y_dope_project/6.25Per-Y-HfO2-2x2x2/berry-scf-results/berry_PlusP_6.25Y_HfO2_12651723/berry_PlusP_6o_DEN.nc" \
  --minus-den "/home/yanan/Material/Y_dope_project/6.25Per-Y-HfO2-2x2x2/berry-scf-results/berry_MinusP_6.25Y_HfO2_12397618/berry_MinusP_6o_DEN.nc" \
  --plus-in   "/home/yanan/Material/Y_dope_project/6.25Per-Y-HfO2-2x2x2/berry-scf-results/berry_PlusP_6.25Y_HfO2_12651723/berry_PlusP_6.25Per_Y_HfO2_2x2x2.in" \
  --minus-in  "/home/yanan/Material/Y_dope_project/6.25Per-Y-HfO2-2x2x2/berry-scf-results/berry_MinusP_6.25Y_HfO2_12397618/berry_MinusP_6.25Per_Y_HfO2_2x2x2.in" \
  --units ang --outdir 2_DEN_Figures_SHARP_6.25Y --write-cube
############################################################################
Notes:
- Requires 'cut3d' in PATH if you pass *_DEN without .nc
...
def read_abinit_den_nc(path_den_nc: str) -> DensityGrid:
    ds = nc.Dataset(path_den_nc)

    if "density" not in ds.variables:
        raise ValueError(f"'density' not in {path_den_nc}. Vars: {list(ds.variables.keys())[:30]}")

    den = np.array(ds.variables["density"][:], dtype=float)
    den = np.squeeze(den)
    if den.ndim != 3:
        raise ValueError(f"After squeeze expected 3D grid, got shape {den.shape} from {path_den_nc}")

    if "primitive_vectors" not in ds.variables:
        raise ValueError(f"'primitive_vectors' not found in {path_den_nc}")
...
def _looks_like_netcdf(path: str) -> bool:
    # Quick signature check: NetCDF classic begins with "CDF", netcdf4 is HDF5.
    try:
...
def convert_den_to_netcdf_with_cut3d(path_den: str, out_nc: str, formatted_flag: int = 1) -> None:
    """
    Convert ABINIT *_DEN -> NetCDF using cut3d.

    Your cut3d menu shows:
      15 => Write data to netcdf file

    BUT some cut3d builds do NOT ask the "formatted/unformatted (0/1)" question.
    In that case, the 'formatted_flag' line would be consumed as the MENU CHOICE
    (causing choice=1 point interpolation and a crash).

    So we try two dialog flows:
      A) file -> 15 -> out_nc -> 0
      B) file -> formatted_flag -> 15 -> out_nc -> 0
    """
    import os
    import subprocess

    def _run_with_answers(answer_lines):
        answers = "\n".join(answer_lines) + "\n"
        proc = subprocess.run(
            ["cut3d"],
            ...
            check=False,   # we'll inspect output ourselves
        )
        out = proc.stdout.decode("utf-8", errors="ignore") if proc.stdout else ""
        return proc.returncode, out

    # Flow A (most likely for your build): NO formatted/unformatted prompt
    flows = [
        [path_den, "15", out_nc, "0", ""],
        # Flow B (fallback): WITH formatted/unformatted prompt
        [path_den, str(formatted_flag), "15", out_nc, "0", ""],
    ]
    ...
    for lines in flows:
        ...
        rc, out = _run_with_answers(lines)
        last_out = out

        # Success criteria: file created and non-trivial size
        if os.path.isfile(out_nc) and os.path.getsize(out_nc) > 1024:
            return

        # If cut3d clearly went into "point" mode, that flow is wrong; try next
        # (This is exactly what happened to you: menu choice became 1)
        if "Your choice is  1" in out and "Select the coordinate system" in out:
            continue

    # If we get here, both flows failed
    raise RuntimeError(
        "cut3d conversion failed for both dialog flows.\n"
        f"DEN  : {path_den}\n"
        f"OUTNC: {out_nc}\n\n"
        "Last ~2000 chars of cut3d output:\n" + last_out[-2000:]
    )



def read_den_auto(path_den_or_nc: str, outdir: str):
    import os
    if not os.path.isfile(path_den_or_nc):
        raise FileNotFoundError(f"Not found: {path_den_or_nc}")

    # If already .nc, read directly
    if path_den_or_nc.endswith(".nc"):
        return read_abinit_den_nc(path_den_or_nc)

    base = os.path.basename(path_den_or_nc)
    out_nc = os.path.join(outdir, base + ".nc")

    if not os.path.isfile(out_nc):
        # try binary first
        try:
            convert_den_to_netcdf_with_cut3d(path_den_or_nc, out_nc, formatted_flag=1)
        except RuntimeError:
            # then try formatted
            convert_den_to_netcdf_with_cut3d(path_den_or_nc, out_nc, formatted_flag=0)

    return read_abinit_den_nc(out_nc)
...
def plot_heatmap(path_base: str,
    ...
    img = data2d.T  # transpose for imshow with [xmin,xmax,ymin,ymax]
...
def main():
    ap = argparse.ArgumentParser(description="2D heatmaps & VESTA cube export from ABINIT DEN (+P/-P).")

    ap.add_argument("--plus-den", required=True, help="Path to +P DEN (either *_DEN.nc or *_DEN)")
    ap.add_argument("--minus-den", required=True, help="Path to -P DEN (either *_DEN.nc or *_DEN)")
    ...
    ap.add_argument("--outdir", default="2_DEN_Figures_SHARP_6.25Y", help="Base output directory name")
    ...
    # Ensure output folder includes 6.25Y
    outdir = args.outdir.rstrip("/").rstrip("_")
    if TAG_OUTDIR.lower() not in outdir.lower():
        outdir = f"{outdir}_{TAG_OUTDIR}"
    os.makedirs(outdir, exist_ok=True)

    # NEW: auto-read .nc OR convert *_DEN -> .nc with cut3d
    plus = read_den_auto(args.plus_den, outdir)
    minus = read_den_auto(args.minus_den, outdir)
    ...
    if args.write_cube:
        ...
        write_gaussian_cube(
            ...
            comment1="rho(+P) from ABINIT DEN (auto netcdf if needed)",
        ...
        write_gaussian_cube(
            ...
            comment1="Delta rho = rho(+P) - rho(-P) (auto netcdf if needed)",
            comment2=f"Units: {unit_label}",
        )
