#!/usr/bin/env python3
"""
Advanced VBM cube comparison for ferroelectric ±P states.

This version supports:
1. python3 2vbm_difference_advanced.py \
    --batch-all \
    --base-dir ./Cube \
    --radius 1.8 \
    --save-npy
...
2. Batch-all mode for:
   ...
   using VBM k-point / band indices from your summary.


python3 2vbm_difference_advanced.py \
    --batch-all \
    --base-dir ./Cube \
    --radius 1.8 \
    --save-npy
...
- This script compares VBM cube files for PlusP vs MinusP.
"""
...
SYSTEMS = [
    ...
    # ----- Sc -----
    {
        ...
        "plus_k": 1, "plus_b": 383,
        "minus_k": 7, "minus_b": 383,
    },
    # ----- Y -----
    {
        "dopant": "Y", "conc": "6.25%", "tag": "6p25Y",
        "plus_k": 1, "plus_b": 383,
        "minus_k": 1, "minus_b": 383,
    },
    # ----- Zr -----
    {
...
def analyze_pair(
    ...
    with open(os.path.join(outdir, "summary.txt"), "w") as f:
        f.write("Advanced VBM difference analysis\n")
        f.write("================================\n\n")
...
def default_cube_names_from_system(system):
    ...
    cube1 = f"PlusP_{tag}_VBM_k{plus_k}_b{plus_b}.cube"
    cube2 = f"MinusP_{tag}_VBM_k{minus_k}_b{minus_b}.cube"

    label1 = f"PlusP_VBM_{tag}"
    label2 = f"MinusP_VBM_{tag}"

    outdir = f"2vbm_{tag}_adv_out"
...
def run_batch_all(base_dir: str, radius: float, save_npy: bool, master_results_dir: str) -> None:
    missing: List[str] = []
    completed = 0
...
def main() -> None:
    ...
    # single-pair mode
    parser.add_argument("--cube1", help="First cube file, usually PlusP VBM cube")
    parser.add_argument("--cube2", help="Second cube file, usually MinusP VBM cube")
    parser.add_argument("--label1", default="State1")
    parser.add_argument("--label2", default="State2")
    parser.add_argument("--outdir", default="vbm_adv_out")

    # batch mode
    parser.add_argument("--batch-all", action="store_true", help="Run all Sc/Y/Zr cases at 3.125, 6.25, 9.375")
    ...
    # ============================================================
    # Create master results folder in the SAME folder as this script
    # ============================================================
    script_dir = os.path.dirname(os.path.abspath(__file__))
    master_results_dir = os.path.join(script_dir, "2vbm_difference_advanced_Results")
    ...
    # Single-pair mode: put output inside the master results folder
